...
<!-- <section class="pdt-section pdt-section13 p-0 my50" id="products"> -->
<section class="pdt-section pdt-section13 p-0 my50" id="order">
   <div class="container" id="products">
      <div class="pdtHeading pb-5">
         <h3 class="m-auto text-center " id="shop"><?= $updateContent["shopTitle"] ?></h3>
      </div>
      <div class="product-wrapper row gx-3">
         ...
         ?>
                     <div class="product-section product-section13 col-lg-4 col-12" data-product="product<?php echo $i; ?>" data-prod-id="<?= $product['id'] ?>" data-product-category="<?= $product['category'] ?>" data-product-title="<?= $product['name'] ?>" data-product-alias="<?= $product['alias'] ?>" data-product-description="<?= $product['description'] ?>" data-product-price-orignal="<?= $priceMin ?>" data-product-price="<?= $priceMin ?>" data-product-shipping="<?= $product['ssShipping'] ?>" <?php if ($product['billingModel'] != 1) { ?> data-product-rbllprice="<?= $product['trialRebillPrice'] ?>" data-product-trlshipping="<?= $product['trialShipping'] ?>" data-product-cntntyprice="<?= $product['continuityPrice'] ?>" data-product-cntntyshipping="<?= $product['continuityShipping'] ?>" <?php } ?> data-product-billmodel="<?= $product['billingModel'] ?>" data-product-MultiPrice="<?= $product['straightSaleMultiPrice'] ?>" data-product-id="product-<?php echo $i; ?>" data-product-size-option="<?= $product['sizeOption'] ?>" data-product-image-link="<?= preg_match('/^(http\:\/\/|https\:\/\/)/', $product['image']) ? $product['image'] : $path['images'] . '/' . $product['image']; ?>">
                        <div class="product-block">
                           <div class="pdtImageWrapper position-relative text-center">
                              <img class="prod_img13" src="<?= preg_match('/^(http\:\/\/|https\:\/\/)/', $product['image']) ? $product['image'] : $path['images'] . '/' . $product['image']; ?>">
                              <a href="javascript:void(0);" class="btn_shop btn_shop13 shop-btn-color" id="btn_shop"><?= $updateContent['buttonName'] ?></a>
                           </div>
                           <div class="product-details">
                              ...
                              <div class="pdt-price-wrapper">
                                 ...
                                 if ($product['billingModel'] == 1 && $product['straightSaleMultiPrice'] == 'no') { ?>
                                    <p class="prod_price_common prod_price13"><span>$<?= $product['ssPrice'] ?></span></p>
                                 <?php }
                                 if ($product['billingModel'] == 1 && $product['straightSaleMultiPrice'] == 'yes') {
                                    $prod_variant_count = count($product['shop_option']);
                                 ?>
                                    <p class="prod_price_common prod_price13">
                                       <span>
                                 ...
                                 if ($product['billingModel'] == 2 || $product['billingModel'] == 8) { ?>
                                    <p class="prod_price_common prod_price13"><span>$<?= $product['trialPrice']; ?> + <?= $product['trialShipping']; ?> for shipping</p>
                                 <?php }
                                 if ($product['billingModel'] == 3) { ?>
                                    <p class="prod_price_common prod_price13"><span>$<?= $product['continuityPrice']; ?> <?php if ($pageConfig['displayShippingPrice'] == "yes") { ?>+ <?= $product['continuityShipping']; ?> for shipping<?php } ?></span></p>
                                 <?php }
                                 if ($product['billingModel'] == 4) { ?>
                                    <p class="prod_price_common prod_price13"><span>$<?= $product['trialShipping'] ?></span></p>
                                 <?php }
