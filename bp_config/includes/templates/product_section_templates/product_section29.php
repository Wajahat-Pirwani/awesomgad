<?php ob_start(); ?>
<section class="pdt-section  pdt-section29 p-0 my50" id="order">
  <div class="container" id="products">
    <div class="product-wrapper row">
      ...
      ?>
              <div class="product-section product-section29 col-lg-4 col-12" data-product="product<?php echo $i; ?>" data-prod-id="<?= $product['id'] ?>" data-product-category="<?= $product['category'] ?>" data-product-title="<?= $product['name'] ?>" data-product-alias="<?= $product['alias'] ?>" data-product-description="<?= $product['description'] ?>" data-product-price-orignal="<?= $priceMin ?>" data-product-price="<?= $priceMin ?>" data-product-shipping="<?= $product['ssShipping'] ?>" <?php if ($product['billingModel'] != 1) { ?> data-product-rbllprice="<?= $product['trialRebillPrice'] ?>" data-product-trlshipping="<?= $product['trialShipping'] ?>" data-product-cntntyprice="<?= $product['continuityPrice'] ?>" data-product-cntntyshipping="<?= $product['continuityShipping'] ?>" <?php } ?> data-product-billmodel="<?= $product['billingModel'] ?>" data-product-MultiPrice="<?= $product['straightSaleMultiPrice'] ?>" data-product-id="product-<?php echo $i; ?>" data-product-size-option="<?= $product['sizeOption'] ?>" data-product-image-link="<?= $path['images'] ?>/<?= $product['image'] ?>">


                <div class="product-block">
                  <div class="product-image">
                    <img class="prod_img29" src="<?= $path['images'] ?>/<?= $product['image'] ?>">
                  </div>
                  <div class="product-details">
                    <?php if ($pageConfig['displayBillingModel'] == 'yes') { ?>
                      <p class="prod_category29">
                        <span>
                          <?php
                    ...
                    <span class="prod-price">
                      ...
                      if ($product['billingModel'] == 1 && $product['straightSaleMultiPrice'] == 'no') { ?>
                        <p class="prod_price_common prod_price29"><span class="">$<?= $product['ssPrice'] ?></span></p>
                      <?php }
                      if ($product['billingModel'] == 1 && $product['straightSaleMultiPrice'] == 'yes') {
                        $prod_variant_count = count($product['shop_option']);
                      ?>
                        <p class="prod_price_common prod_price29"><span class="">
                      ...
                      if ($product['billingModel'] == 2 || $product['billingModel'] == 8) { ?>
                        <p class="prod_price_common prod_price29"><span>$<?= $product['trialPrice']; ?> + <?= $product['trialShipping']; ?> for shipping</p>
                      <?php }
                      if ($product['billingModel'] == 3) { ?>
                        <p class="prod_price_common prod_price29"><span>$<?= $product['continuityPrice']; ?> <?php if ($pageConfig['displayShippingPrice'] == "yes") { ?>+ <?= $product['continuityShipping']; ?> for shipping<?php } ?></span></p>
                      <?php }
                      if ($product['billingModel'] == 4) { ?>
                        <p class="prod_price_common prod_price29"><span class="">$<?= $product['trialShipping'] ?></span></p>
                      <?php }
                    ...
                    <h5 class="product-title product-name29"><?= $product['name'] ?></h5>
                    <a href="javascript:void(0);" class="btn_shop btn_shop29" id="btn_shop"><button class="btn btn-primary btn-shop shop-btn-color"><?= $updateContent['buttonName'] ?></button></a>
                  </div>
                </div>
