      ...
      <?php if ($pageConfig['displayRelatedProducts'] == 'yes') { ?>
         <div class="container pdt-section-popular" id="related_prods">
            ...
            <div class="product-wrapper  row">
               ...
               ?>
                           <div class="product-section   col-lg-4 col-12" data-product="product<?php echo $i; ?>" data-prod-id="<?= $product['id'] ?>" data-product-sticky-id="<?= $product['stickyId'] ?>" data-product-category="<?= $product['category'] ?>" data-product-title="<?= $product['name'] ?>" data-product-alias="<?= $product['alias'] ?>" data-product-description="<?= $product['description'] ?>" data-product-price-orignal="<?= $product['ssPrice'] ?>" data-product-price="<?= $product['ssPrice'] ?>" data-product-shipping="<?= $product['ssShipping'] ?>" <?php if ($product['billingModel'] != 1) { ?> data-product-rbllprice="<?= $product['trialRebillPrice'] ?>" data-product-trlPrice="<?= $product['trialPrice'] ?>" data-product-trlshipping="<?= $product['trialShipping'] ?>" data-product-cntntyprice="<?= $product['continuityPrice'] ?>" data-product-cntntyshipping="<?= $product['continuityShipping'] ?>" <?php } ?> data-product-billmodel="<?= $product['billingModel'] ?>" data-product-MultiPrice="<?= $product['straightSaleMultiPrice'] ?>" data-product-id="product-<?php echo $i; ?>" data-product-size-option="<?= $product['sizeOption'] ?>" data-product-image-link="<?= preg_match('/^(http\:\/\/|https\:\/\/)/', $product['image']) ? $product['image'] : $path['images'] . '/' . $product['image']; ?>">
                              <div class="product-block">
                                 ...
                                 if ($product['billingModel'] == 1 && $product['straightSaleMultiPrice'] == 'no') { ?>
                                    <p class="prod_price_common prod_price"><span class="primary-text-color"><span class="currency_symbol">€</span><?= $product['ssPrice'] ?></span></p>
                                 <?php }
                                 if ($product['billingModel'] == 1 && $product['straightSaleMultiPrice'] == 'yes') {
                                    $prod_variant_count = count($product['shop_option']);
                                 ?>
                                    <p class="prod_price_common prod_price"><span class="primary-text-color">
                                          $<?= $product['shop_option']['shop_option1']['option_price'] ?> - <span class="currency_symbol">€</span><?= $product['shop_option']['shop_option' . $prod_variant_count]['option_price'] ?>
                                       </span></p>
                                 <?php }
                                 if ($product['billingModel'] == 2 || $product['billingModel'] == 8) { ?>
                                    <p class="prod_price_common prod_price"><span class="primary-text-color"><span class="currency_symbol">€</span><?= $product['trialShipping'] ?></span></p>
                                 <?php }
                                 if ($product['billingModel'] == 3) { ?>
                                    <p class="prod_price_common prod_price"><span class="primary-text-color"><span class="currency_symbol">€</span><?php echo sprintf('%.2f', $product['continuityPrice'] +  $product['continuityShipping']); ?></span></p>
                                 <?php }
                                 ...
                                 if ($product['billingModel'] == 6) { ?>
                                    <p><span class="primary-text-color"><span class="currency_symbol">€</span><?= $product['trialShipping'] ?></span></p>
                                 <?php }
