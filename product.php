...
<body>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var productPrice = sessionStorage.getItem('data_product_price');
            $("#subtotal_price").html(productPrice);
            $("#single_pdt_price").html(productPrice);
            ...
            if (bill_model == 1 && multiPrice == 'no') {
                ...
                sessionStorage.setItem('data_total_price', productPrice);
            }
    ...
    <script type="text/javascript">
        ...
        function AddtoCart() {
            ...
            if (singleProduct.Id === 'product-7') {
                var cartArray = JSON.parse(sessionStorage.getItem("cartPdtArr"));
                var pkgFlag = 0;
                if (cartArray.length > 0) {
                    console.log("Cart has items");
                    cartArray.forEach(cartProduct => {
                        if (cartProduct.Id === "product-6") {
                            pkgFlag = 1;
                            return 0;
                        }
                    });
                    if (flag == 0 && pkgFlag == 0) {
                        ...
                        $('#error_modal').show(function() {
                            ...
                            $('#error_pop').append(
                                ...
                                "Another Package is already added to cart. Please remove that first!" +
                                '</p>' +
                                '</div>'
                            );
                        });
                    ...
                    $('#error_modal').show(function() {
                        ...
                        $('#error_pop').append(
                            ...
                            '<i class="fas fa-times me-2"></i>' +
                            '<p class="error-msg p-0 m-0">' +
                            "Cart is Empty Please add an item to cart first before adding Premium Plan" +
                            '</p>' +
                            '</div>'
                        );
                    });
            ...
            } else if (singleProduct.Id === 'product-6') {
                var cartArray = JSON.parse(sessionStorage.getItem("cartPdtArr"));
                var pkgFlag = 0;
                if (cartArray.length > 0) {
                    cartArray.forEach(cartProduct => {
                        if (cartProduct.Id === "product-7") {
                            pkgFlag = 1;
                            return 0;
                        }
                    });
